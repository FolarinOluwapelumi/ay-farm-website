...
export function FocusRail({
  ...
  return (
    <div
      className={cn(
        'group relativeflex h-175 w-full flex-col overflow-hidden bg-green-500 text-green-300 outline-none select-none overflow-x-hidden',
        className
      )}
    ...
    >
      ...
      <div className="absolute inset-0 z-0 pointer-events-none">
        <AnimatePresence mode="popLayout">
          ...
          >
            ...
            />
            <div className="absolute inset-0 bg-gradient-to-t from-green-800 via-green-400 to-transparent" />
          </motion.div>
        </AnimatePresence>
      </div>
      ...
      <div className="relative z-10 flex flex-1 flex-col justify-center px-4 md:px-8">
          ...
          {visibleIndices.map((offset) => {
            ...
            return (
              <motion.div
                key={absIndex}
                className={cn(
                  'absolute aspect-[3/4] w-[260px] md:w-[300px] rounded-2xl border-t border-white/20 bg-neutral-500 shadow-2xl transition-shadow duration-300',
        ...
        <div className="mx-auto mt-12 flex w-full max-w-4xl flex-col items-center justify-between gap-6 md:flex-row pointer-events-auto">
          <div className="flex flex-1 flex-col items-center text-center md:items-start md:text-left h-50 justify-center">
            <AnimatePresence mode="wait">
              ...
              >
                {activeItem.meta && (
                  <span className="text-xl font-bold uppercase tracking-wider text-green-600">
                    {activeItem.meta}
                  </span>
                )}
                <h2 className="text-2xl font-bold tracking-tight md:text-4xl text-green-600">
                  {activeItem.title}
                </h2>
                {activeItem.description && (
                  <p className="max-w-md text-green-400">
                    {activeItem.description}
                  </p>
                )}
              </motion.div>
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-1 rounded-full bg-green-600 p-1 ring-1 ring-white/10 backdrop-blur-md">
              <button
                onClick={handlePrev}
                className="rounded-full p-3 text-white transition hover:bg-white/10 hover:text-white active:scale-95"
                aria-label="Previous"
              >
                <ChevronLeft className="h-5 w-5" />
              </button>
              <span className="min-w-[40px] text-center text-xs font-mono text-white">
                {activeIndex + 1} / {count}
              </span>
              <button
                onClick={handleNext}
